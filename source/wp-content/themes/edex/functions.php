...
function edexScripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_register_script('edexscript', get_template_directory_uri() . '/js/global.min.js', array(), '0.1.0', true);
...
function edexStyles()
{
    wp_register_style('pluginscss', get_template_directory_uri() . '/css/plugins.min.css', array(), '0.1.0', 'all');
    wp_enqueue_style('pluginscss'); // Enqueue it!

    wp_register_style('edexcss', get_template_directory_uri() . '/style.css', array(), '0.1.0', 'all');
...
function man_register_custom_widgets() {
    register_widget( 'My_Widget_Recent_Posts' );
}
add_action( 'widgets_init', 'man_register_custom_widgets' );
/**
...
class My_Widget_Recent_Posts extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array('classname' => 'my_widget_recent_entries', 'description' => __( "Your site&#8217;s most recent Posts.") );
    ...
    public function widget($args, $instance) {
        $cache = array();
        if ( ! $this->is_preview() ) {
            $cache = wp_cache_get( 'my_widget_recent_entries', 'widget' );
        }
            ...
            <ul>
                <?php while ( $r->have_posts() ) : $r->the_post(); ?>
                    <li>
                        ...
                        <span class="post-date"><?php echo get_the_date(); ?> by <?php echo get_the_author(); ?></span>
                    </li>
                <?php endwhile; ?>
            </ul>
        ...
        if ( ! $this->is_preview() ) {
            ...
            wp_cache_set( 'my_widget_recent_entries', $cache, 'widget' );
        } else {
            ob_end_flush();
        }
    }
    ...
    public function flush_widget_cache() {
        wp_cache_delete('my_widget_recent_entries', 'widget');
    }
